...
Patch202: httpd-2.2.15-CVE-2011-3348.patch
Patch203: httpd-2.2.15-CVE-2011-3368.patch
License: ASL 2.0
...
%package -n mod_file_cache
Group: System Environment/Daemons
Summary: mod_file_cache provides two techniques for caching frequently requested static files.
Epoch: 1
Requires(pre): httpd
Requires: httpd = 0:%{version}-%{release}, httpd-mmn = %{mmn}

%description -n mod_file_cache
mod_file_cache provides two techniques for caching frequently requested static files. Through configuration directives, you can direct mod_file_cache to either open then mmap() a file, or to pre-open a file and save the file's open file handle. Both techniques reduce server load when processing requests for these files by doing part of the work (specifically, the file I/O) for serving the file when the server is started rather than during each request.
...
%patch202 -p1 -b .cve3348
%patch203 -p1 -b .cve3368
...
for f in ssl.conf welcome.conf manual.conf mod_file_cache.conf; do
...
%files -n mod_file_cache
%defattr(-,root,root)
%{_libdir}/httpd/modules/mod_file_cache.so
%config(noreplace) %{_sysconfdir}/httpd/conf.d/mod_file_cache.conf
...
%changelog
* Thu Feb 16 2012 Jess Portnoy <jess.portnoy@kaltura.com> 2.2.15-9.4
- Added build and packaging for mod_cache_file seeing how the Kaltura platform relies on it
