<?php 
	...
	**/
	...
	{
		$svn_cmd = "svn -q --username $user --password $pass export --force $repository $destination";
		echo "$svn_cmd\n";
		$result = exec($svn_cmd);
		if ($result != '') {
			echo "Failed svn export of $repository to $destionation failed. Error: $result\n";
			die(1);
		}	
	}
	...
	{	
		if ($group['get'][0] == '*') {
			svn_export($global['svn_repo'] . $group['svn_path'], $global['svn_user'], $global['svn_pass'], $base_dir . $group['local_path']);
		}
		else {
			foreach($group['get'] as $current) {
				svn_export($global['svn_repo'] . $group['svn_path'] . $current, $global['svn_user'], $global['svn_pass'], $base_dir . $group['local_path'] . $current);
			}
		}
	}	
